...
class Myo(MyoRaw):
	...
	if (pose == 0.0):
		try:
			subprocess.call("rm -f als_override_off", shell=True)
		except:
			pass 
		subprocess.call("touch  als_override_on", shell=True)
		subprocess.call("sudo ./als_read 150 250 on", shell=True)
	 	print ("Pose 1 - LED UP")
	elif (pose == 1.0):
		try:
			subprocess.call("rm -f als_override_on", shell=True)
		except:
			pass 
		subprocess.call("touch  als_override_off", shell=True)
		subprocess.call("sudo ./als_read 150 250 off", shell=True)
		print ("LED DOWN")
	else:
		print ("Ignore Case : 2/3")
...
if __name__ == '__main__':
    ...
    subprocess.call("sudo ./als_init", shell=True)
    def page(pose):
        if pose == 5:
